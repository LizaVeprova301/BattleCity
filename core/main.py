...
class Tank:
    ...
    def player_move(self, old_x, old_y):
        if keys[pygame.K_SPACE] and self.delay == 0:
            tanks.append(Bullet(self.aim, self.x, self.y, self, self.bullet))
            if self.tank_type == 'fast_bullet':
                self.delay = 30
            else:
...
def MenuScreen():
    ...
    if level1_button.draw(screen):
        f = open("maps/road1.txt", encoding="UTF-8")
    ...
    if level2_button.draw(screen):
        f = open("maps/road2.txt", encoding="UTF-8")
        for line in f.readlines():
            terrain_grid.append(list(map(int, line.split())))
        terrain_grid = [*zip(*terrain_grid)]
        for i in range(50):
    ...
    if level3_button.draw(screen):
        f = open("maps/road3.txt", encoding="UTF-8")
        for line in f.readlines():
            terrain_grid.append(list(map(int, line.split())))
        terrain_grid = [*zip(*terrain_grid)]
        for i in range(50):
    ...
    if level4_button.draw(screen):
        f = open("maps/road4.txt", encoding="UTF-8")
        for line in f.readlines():
            terrain_grid.append(list(map(int, line.split())))
        terrain_grid = [*zip(*terrain_grid)]
        for i in range(50):
...
def show_game_over_screen():
    ...
    text_surface = font.render(GAME_OVER, True, RED)
...
def show_game_win_screen():
    screen.fill(BLACK)
    font = pygame.font.Font(FONT_NAME, 30)
    text_surface = font.render(GAME_WIN, True, GREEN)
    text_rect = text_surface.get_rect()
    text_rect.center = CENTER_SCREEN_POS
    screen.blit(text_surface, text_rect)
    pygame.display.update()
...
def init_game_map():
    ...
    tanks = [player_tank, base] + boosters
    ...
    for i in range(5):
        ...
        tanks.append(Tank(enemy_x * 32, enemy_y * 32, False, tank_type="simple"))

    tanks.append(game_map)
